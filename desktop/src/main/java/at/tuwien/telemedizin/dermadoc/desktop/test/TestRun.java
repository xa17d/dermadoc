    ...
    private void run() {
        ...
        login.login(new AuthenticationListener() {
            ...
            public void onAuthenticationComplete(AuthenticationToken token) {
                ...
                CaseData text2 = new TextMessage(-2l, Calendar.getInstance(), p, "thank you :)");

                try {
                    IRestCaseService service = new RestCaseService(token);

                    Thread.sleep(2000);

                    service.postNewCase(nullListenerC, c);

                    Thread.sleep(1000);

                    service.postCaseData(nullListenerCd, c, anamnesis);

                    Thread.sleep(3000);

                    service.postCaseData(nullListenerCd, c, text);
